...
# Chord list ([chord or note name], note type, duration) (ticks)
chords = [
    (['C4','G4'], "móc đơn", 240), (['C4','G4'], "móc đơn", 240), ("D#4","móc đơn", 240), ("E4","móc đơn", 240),
    (['C4','G4'], "móc đơn", 240), (['C4','G4'], "móc đơn", 240), ("D#4","móc đơn", 240), ("E4","móc đơn", 240), #1.5
...
for chord_data in chords:
    if chord_data[0] == "lặng": # Nghỉ
        ...
        for note in notes if isinstance(notes, list) else [notes]:
            midi_note = note_to_midi[note] - 12  # Back 1 octave for lower sound
